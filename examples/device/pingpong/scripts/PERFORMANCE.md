...
8. **Multi-channel / multi-worker scaling test** (currently fixed at 1/1).
   Worker is 99.9 % idle, so the *single* proxy can clearly handle
   multiple GPU producers — quantify it.
...
2. **Both binaries currently use `do_put_sync`** in the kernel (changed
   during the proxy enablement work). The original UCX-direct number
   you'd find in the upstream repo used `do_put_async` and would be
   lower for small messages.
3. **The flat 13.05 ms across sizes for the proxy is suspicious enough
   ...
   and/or checking `do_put_sync` semantics in the proxy facade would be
   a good belt-and-suspenders before publishing.
4. **One missing data point for the demo:** UCX-direct's `prep+submit` /
