...
(the elasticity guarantee for `connect_ranks` / `disconnect_ranks`). It also adds a revive path
(`resetProxyRankChannels`) so a reconnecting rank's stale latch is cleared.
...
```
Expect all pass, including `ResetRankChannelsRevivesLatchedChannel` (the latch→reset→revive cycle),
`ResetRankChannels{NoopWhenEncodingDisabled,RejectsOutOfRange}`, and
`InitRejectsChannelCountNotMultipleOfChannelsPerRank`.
...
|---|---|---|
| 0→1 | 4 → 8 | **expansion**: `connect_ranks` on new ranks |
| 2 | remove rank 5, **kill** rank 6 (`-6`) | **contraction + fault**: `disconnect_ranks(5)`; rank 6 self-kills → its rings may latch. Survivors (0-4,7) must still pass |
| 3 | re-add 5 & 6 | **revive**: `resetProxyRankChannels` clears rank 6's stale latch |
...
3. **Revive fired:** with `NIXL_LOG_LEVEL=DEBUG`, the re-add phase logs
   `ProxyRuntime::resetRankChannels` and `ProxyWorker::resetChannel: channel=… discarded=…`.
...
`#ifdef NIXL_GPU_DEVICE_BACKEND_PROXY`-gated and `resetProxyRankChannels` is a runtime no-op without a
proxy runtime, so behavior must match pre-change.
