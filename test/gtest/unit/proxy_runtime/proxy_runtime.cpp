...
static WorkRing
copyDeviceWorkRing(const ProxyChannelView &view) {
    WorkRing ring{};
...
static ProxySubmission *
hostRecordsFromDeviceAlias(ProxySubmission *records_dev) {
    cudaPointerAttributes attrs{};
    EXPECT_EQ(cudaPointerGetAttributes(&attrs, records_dev), cudaSuccess);
    return static_cast<ProxySubmission *>(
...
TEST_F(ProxyRuntimeTest, DeviceChannelViewsPopulated) {
    ...
    for (uint32_t i = 0; i < 3; ++i) {
        ...
        const WorkRing ring = copyDeviceWorkRing(views[i]);
        EXPECT_NE(ring.records, nullptr);
        EXPECT_NE(ring.producer_ticket, nullptr);
...
TEST_F(ProxyRuntimeTest, WorkerSubmitsPreparedTransportDescriptors) {
    ...
    const WorkRing ring = copyDeviceWorkRing(runtime_.deviceChannelViews()[0]);
...
TEST_F(ProxyRuntimeTest, WorkerSubmitsPreparedAtomicAddDescriptor) {
    ...
    const auto &prepared = backend_.submissions_.front();
    EXPECT_EQ(prepared.op_idx, 11u);
    EXPECT_EQ(prepared.opcode, ProxyOpcode::ATOMIC_ADD);
    EXPECT_EQ(prepared.channel_id, 0u);
    EXPECT_EQ(prepared.remote.mem_type, DRAM_SEG);
    EXPECT_EQ(prepared.remote.desc.addr, 0x2008u);
