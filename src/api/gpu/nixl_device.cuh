...
nixlGpuGetXferStatus(nixlGpuXferStatusH &xfer_status) {
    return nixl::gpu::dispatch::get_xfer_status<level>(xfer_status);
}
...
nixlPut(const nixlMemViewElem &src,
    ...
    return nixl::gpu::dispatch::put<level>(src, dst, size, channel_id, flags, xfer_status);
}
...
nixlAtomicAdd(uint64_t value,
              ...
              uint64_t flags = 0,
              nixlGpuXferStatusH *xfer_status = nullptr) {
    return nixl::gpu::dispatch::atomic_add<level>(value, counter, channel_id, flags, xfer_status);
}

__device__ inline void *
nixlGetPtr(nixlMemViewH mvh,
           size_t index) {
    return nixl::gpu::dispatch::get_ptr(mvh, index);
}
