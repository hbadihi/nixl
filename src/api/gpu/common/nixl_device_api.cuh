...
validate_proxy_counter(const nixlMemViewElem &counter,
    ...
    return memview->context.shutdown_word == nullptr ? NIXL_ERR_INVALID_PARAM : NIXL_SUCCESS;
}
...
put(const nixlMemViewElem &src,
    ...
    if (dst_view.backend == detail::MemViewBackend::UCX) {
        ...
        status = detail::validate_proxy_put(backend_src, backend_dst, dst_memview);
        ...
        if constexpr (level == nixl_gpu_level_t::GRID) {
            ...
            status = proxy_impl::put<level>(dst_memview->context,
                                            backend_src,
                                            backend_dst,
                                            size,
                                            channel_id,
    ...
    if (view.backend == detail::MemViewBackend::UCX) {
        ...
        if constexpr (level == nixl_gpu_level_t::GRID) {
            ...
            status = proxy_impl::atomic_add<level>(
                memview->context, value, backend_counter, channel_id, flags, xfer_status);
        }
        status_backend = nixlDeviceXferStatusBackend::PROXY;
    }
    detail::write_footer<level>(xfer_status, status, status_backend);
