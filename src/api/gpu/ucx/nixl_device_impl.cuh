...
struct UcpDeviceLevel;

template<>
struct UcpDeviceLevel<nixl_gpu_level_t::THREAD> {
    static constexpr ucs_device_level_t value = UCS_DEVICE_LEVEL_THREAD;
};

template<>
struct UcpDeviceLevel<nixl_gpu_level_t::WARP> {
    static constexpr ucs_device_level_t value = UCS_DEVICE_LEVEL_WARP;
};

template<>
struct UcpDeviceLevel<nixl_gpu_level_t::BLOCK> {
    static constexpr ucs_device_level_t value = UCS_DEVICE_LEVEL_BLOCK;
};

template<>
struct UcpDeviceLevel<nixl_gpu_level_t::GRID> {
...
get_xfer_status(nixlGpuXferStatusH &xfer_status) {
    const auto status = ucp_device_progress_req<UcpDeviceLevel<level>::value>(
...
atomic_add(uint64_t value,
    ...
    const auto status = ucp_device_counter_inc<UcpDeviceLevel<level>::value>(value,
                                                                              mem_list,
                                                                              counter.index,
                                                                              counter.offset,
                                                                              channel_id,
