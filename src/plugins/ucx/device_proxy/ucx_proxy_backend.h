...
class nixlUcxProxyBackend : public DeviceProxyBackendAdapter {
    public:
        explicit nixlUcxProxyBackend(nixlUcxEngine *engine = nullptr,
    ...
    private:
        nixl_status_t
        submitPut(const PreparedProxySubmission &submission, uint64_t &request_token);

        nixl_status_t
        submitAtomicAdd(const PreparedProxySubmission &submission, uint64_t &request_token);
