...
nixlDeviceMemViewAllocation::create(nixlDeviceMemViewBackend backend,
    ...
    if (backend_handle == nullptr ||
        (backend != nixlDeviceMemViewBackend::UCX && backend != nixlDeviceMemViewBackend::PROXY)) {
    ...
    if (cudaMemcpy(device_wrapper, &host_wrapper, sizeof(host_wrapper), cudaMemcpyHostToDevice) !=
        cudaSuccess) {
        return NIXL_ERR_BACKEND;
    }
