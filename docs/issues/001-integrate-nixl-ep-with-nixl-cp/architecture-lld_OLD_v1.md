...
> Replacement: architecture-lld.md
...
**Input:** `void *ctx` returned by `nixlAgent::getProxyDeviceContext()` after `createBackend("UCX", ...)` in a proxy build.
**Output:** `cudaSuccess` when the device-side proxy context is published and subsequent GPU Device API proxy calls can load it.
**Error contracts:**
...
**Input:** No arguments; called during proxy teardown before agent/proxy resources are released.
**Output:** `cudaSuccess` when the device-side proxy context is cleared.
**Error contracts:**
...
sequenceDiagram
    ...
    A-->>B: ctx
    B->>C: nixl_ep_proxy_publish_context(ctx)
    C->>G: nixlProxyPublishContext(ctx)
    G-->>C: cudaSuccess
    C-->>B: cudaSuccess
    D->>C: nixl_ep_proxy_clear_context()
    C->>G: nixlProxyClearContext()
    C-->>D: cudaSuccess or teardown warning
```

### Error Handling
Publish happens after backend creation and before memview-backed device operations. A missing or failed publish is an early setup error, not a device-side surprise from `nixlPut` returning `NIXL_ERR_NOT_SUPPORTED`. Clear happens before agent/proxy teardown. The exact C++ mechanism (`std::runtime_error`, `EP_HOST_ASSERT`, or local teardown logging) should match the surrounding EP style.
...
**Input:** Selected proxy build macro, allocation-time lane ceiling from `Buffer::update_memory_buffers(...)`, optional `NIXL_EP_PROXY_CHANNELS` override, existing UCX `init_params`.
...
sequenceDiagram
    ...
    Py->>B: update_memory_buffers(..., lane_ceiling, ...)
...
ep_proxy_runtime_config_v1:
  ...
  required_proxy_channels: <allocation_time_lane_ceiling>
  ucx_num_workers: 1
...
**Input:** The lane ceiling passed through `Buffer::update_memory_buffers(...)`; HT supplies its `num_qps_per_rank` lane requirement and elastic LL supplies its local expert lane count through the same API shape.
...
sequenceDiagram
    ...
    U->>B: Provide allocation-time lane ceiling
    B->>B: required_proxy_channels = lane ceiling
...
ep_proxy_channel_sizing_v1:
  ...
  source: update_memory_buffers_lane_ceiling
  override:
...
**Purpose:** Consume existing CPU proxy and UCX backend capabilities for Phase 1 without changing proxy internals, UCX worker/QP routing, or multi-worker scaling.
...
**Output:** Proxy worker drains work records and submits supported RMA write and atomic add operations through the existing UCX backend/provider.
**Error contracts:**
...
sequenceDiagram
    ...
    W->>R: Drain configured channels
    W->>A: Submit PUT or ATOMIC_ADD
    A->>X: Use existing UCX worker/QP path
    W->>E: Increment or emit proxy activity signal
```

### Error Handling
Phase 1 does not retry failed atomics as if they were idempotent. It also does not infer proxy execution from runtime startup alone. The minimum accepted signal is proxy activity during the HT or LL run being validated.
...
**Input:** Backend selection, rank, proxy runtime config, channel sizing, context publish result, proxy activity signal, LL fallback signal, correctness result, validation path metadata.
...
sequenceDiagram
    ...
    R-->>E: Backend, rank, config, context published
    P-->>E: Proxy activity observed
    opt elastic LL
...
**Output:** HT correctness pass plus `ep_proxy_evidence_v1` showing proxy backend selection, context publish, proxy activity, and accepted validation path metadata.
**Error contracts:**
...
#### Invalid setup tests
**Input:** Negative configurations such as missing proxy context, under-provisioned channels, absent proxy activity, missing LL fallback signal, unsupported HT topology, or undefined reduced-size timeout workaround.
...
The runtime order is strict. `Buffer.update_memory_buffers(...)` establishes the allocation-time lane ceiling. `Buffer::_nixl_agent_init()` derives `required_proxy_channels`, validates `NIXL_EP_PROXY_CHANNELS` if present, configures one proxy worker and enough channels, creates the UCX backend with one worker, publishes the proxy device context, then proceeds through the existing memview creation path. HT and LL kernels continue using backend-agnostic NIXL GPU Device API wrappers. In proxy builds, work is enqueued by channel and drained by the single CPU proxy worker into the existing UCX provider.

Validation is not a separate afterthought. The same implementation must expose deterministic evidence for backend selection, context publish, channel coverage, proxy activity, LL all-RDMA fallback, correctness result, and classification. The `ep_proxy_evidence_v1` record is a validation/test artifact, not a public production API. Existing lower-level proxy tests remain useful for publish, clear, enqueue, and channel behavior; EP-level tests cover selected-backend build/import, lifecycle setup, channel override validation, HT proxy smoke, elastic LL fallback evidence, and independent UCX-direct correctness.

Migration steps are intentionally small and ordered: first update the EP build gate and proxy-only source/link wiring; next add the CUDA publish/clear seam; then guard proxy lifecycle setup in `nixl_ep.cpp`; then add channel override validation; then add evidence hooks and tests. CPU proxy internals, UCX worker/QP routing, multi-worker mapping, performance comparison artifacts, and proxy `nixlGetPtr` peer-pointer restoration remain outside Phase 1.
...
- The concrete proxy activity instrumentation should prefer existing counters or deterministic low-cost logs if available; if none exist, tasks should add the smallest correctness-only signal needed for `ep_proxy_evidence_v1`.
